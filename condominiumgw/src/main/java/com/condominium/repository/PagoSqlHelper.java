package com.condominium.repository;

import java.util.ArrayList;
import java.util.List;
import org.springframework.data.relational.core.sql.Column;
import org.springframework.data.relational.core.sql.Expression;
import org.springframework.data.relational.core.sql.Table;

public class PagoSqlHelper {

    public static List<Expression> getColumns(Table table, String columnPrefix) {
        List<Expression> columns = new ArrayList<>();
        columns.add(Column.aliased("id", table, columnPrefix + "_id"));
        columns.add(Column.aliased("anio", table, columnPrefix + "_anio"));
        columns.add(Column.aliased("mes", table, columnPrefix + "_mes"));
        columns.add(Column.aliased("valor", table, columnPrefix + "_valor"));
        columns.add(Column.aliased("estado", table, columnPrefix + "_estado"));
        columns.add(Column.aliased("fecha_generacion", table, columnPrefix + "_fecha_generacion"));
        columns.add(Column.aliased("fecha_pago", table, columnPrefix + "_fecha_pago"));

        columns.add(Column.aliased("cuota_id", table, columnPrefix + "_cuota_id"));
        return columns;
    }
}
